...
#define ADVANCE_OPT(o,e,step) if ((e) < step) {			\
   WD_DEBUG("cannot advance opt past end\n");			\
    return 0;							\
  } else {							\
    (e) -= step;						\
  ...
  case 15:
    if (*opt != COAP_PAYLOAD_START)
      WD_DEBUG("ignored reserved option delta 15\n");
    return 0;
  case 14:
  ...
  case 15:
    WD_DEBUG("found reserved option length 15\n");
    return 0;
  case 14:
    /* Handle two-byte value: First, the MSB + 269 is stored as delta value.
     * After that, the option pointer is advanced to the LSB which is handled
  ...
  if (length < result->length) {
    WD_DEBUG("invalid option length\n");
    return 0;
  }
...
coap_opt_delta(const coap_opt_t *opt) {
  ...
  switch (n) {
  case 15: /* error */
    WD_WARN("coap_opt_delta: illegal option delta\n");
...
coap_opt_length(const coap_opt_t *opt) {
  ...
  switch (length) {
  case 0x0f:
    WD_DEBUG("illegal option length\n");
    return 0;
  case 0x0e:
    length = (*opt++ << 8) + 269;
    /* fall through */
...
coap_opt_setheader(coap_opt_t *opt, size_t maxlen, 
  ...
  if (delta < 13) {
    ...
    if (maxlen < 2) {
      WD_DEBUG("insufficient space to encode option delta %d", delta);
      return 0;
    }
  ...
  if (length < 13) {
    ...
    if (maxlen < skip + 1) {
      WD_DEBUG("insufficient space to encode option length %d", length);
      return 0;
    }
...
coap_opt_encode(coap_opt_t *opt, size_t maxlen, unsigned short delta,
  ...
  if (!l) {
    WD_DEBUG("coap_opt_encode: cannot set option header\n");
    return 0;
  }
  ...
  if (maxlen < length) {
    WD_DEBUG("coap_opt_encode: option too large for buffer\n");
    return 0;
  }
