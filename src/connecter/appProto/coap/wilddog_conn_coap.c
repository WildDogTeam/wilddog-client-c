...
#define WILDDOG_CONN_COAP_RESPON_IGN 10
typedef enum{
...
inline unsigned int _wilddog_conn_coap_code2int(unsigned int code) 
{
...
STATIC int _wilddog_conn_coap_findC(const char c,const unsigned char *p_buf)
{
	int res = 0 ;
	u32 i;
	for(i=0;p_buf[i] != 0;i++)
...
STATIC int _wilddog_conn_coap_cmd2Typecode(Wilddog_Conn_Cmd_T cmd,u8 *p_type,u8 *p_code,u32 **pp_observe)
{
    int res = 0;
    switch(cmd){
...
inline int _wilddog_conn_coap_noSeparate(Wilddog_Conn_Coap_PacketNode_T *p_node)
{
    return (p_node->d_separate_flag == WILDDOG_CONN_COAPPKT_NO_SEPARATE);
}
inline int _wilddog_conn_coap_noObserve(Wilddog_Conn_Coap_PacketNode_T *p_node)
{
    return ((p_node->d_observer_flag & WILDDOG_CONN_COAPPKT_IS_OBSERVER) == 0);
}
inline int _wilddog_conn_coap_isNotify(Wilddog_Conn_Coap_PacketNode_T *p_node)
{
    return (p_node->d_observer_flag ==  WILDDOG_CONN_COAPPKT_IS_NOTIFY);
}

inline int _wilddog_conn_coap_midEqu(coap_pdu_t* p_node,coap_pdu_t* p_resp)
{
...
STATIC int _wilddog_conn_coap_countpacktsize(Wilddog_Conn_PktSend_T *p_cp_pkt)
{
	...
	if(p_cp_pkt->p_url)
	{
		...
		if(p_url->p_url_path)
		{
			n = _wilddog_conn_coap_findC('/',p_url->p_url_path);
...
inline int _wilddog_conn_coap_nodeAdd
    (
...
Wilddog_Conn_Coap_PacketNode_T *_wilddog_conn_coap_nodeCreat
    (
...
void _wilddog_conn_coap_nodeFree
    (
    Wilddog_Conn_Coap_PacketNode_T **p_node
    )
{
...
inline void _wilddog_conn_coap_nodeRemove
    (
    Wilddog_Conn_Coap_PCB_T *p_pcb,
...
void _wilddog_conn_pkt_free(void **pp_pkt)
{
    ...
    _wilddog_conn_coap_nodeRemove(p_coap_pcb, \
                                    (Wilddog_Conn_Coap_PacketNode_T*)*pp_pkt);
    if(pp_pkt)
    {
        _wilddog_conn_coap_nodeFree((Wilddog_Conn_Coap_PacketNode_T **)pp_pkt);
    }
}
    ...
    )
{
    ...
    p_ndoe = _wilddog_conn_coap_nodeCreat(p_pktSend);
    if(p_ndoe == NULL)
        return WILDDOG_ERR_NULL;
        
    res = _wilddog_conn_coap_nodeAdd(p_coap_pcb,p_ndoe);
    if(res < 0)
        _wilddog_conn_coap_nodeFree(&p_ndoe);
    *pp_pkt_creat = p_ndoe;
    return res;
}
STATIC int _wilddog_conn_coap_authupdata(u8 *p_auth,coap_pdu_t *p_pdu)
{
    ...
    )
{
    ...
    _wilddog_conn_coap_authupdata(p_auth,p_coap);
...
/*@ timeout retransmits 
*  @ freee node and coap packet pdu
*/
...
STATIC int _wilddog_conn_coap_recvRespCheck
    (
    Wilddog_Conn_Coap_PacketNode_T *p_node,
    coap_pdu_t* p_resp
    )
...
STATIC int _wilddog_conn_coap_recvSeparationRespCheck
    (
    Wilddog_Conn_Coap_PacketNode_T *p_node,
    coap_pdu_t *p_resp
    )
{
    if(p_node->p_CoapPkt->hdr->type == COAP_MESSAGE_CON){
        ...
        {
            return WILDDOG_CONN_COAP_RESPON_IGN;
...
/*@ get data */
STATIC int _wilddog_conn_coap_recvObservCheck
    (
    Wilddog_Conn_Coap_PacketNode_T *p_node,
    coap_pdu_t *p_resp
    )
...
/*@ check coap packer and copy payloat delete node  if match 
*/
    ...
    ) 
{
    ...
    LL_FOREACH_SAFE((p_pcb->P_hd),curr,tmp)
    {
        ...
        res = _wilddog_conn_coap_recvRespCheck(curr,p_resp);
        if(res < 0)
            continue ; 
            
        /*@ get observer indx */
        res = _wilddog_conn_coap_recvObservCheck(curr,p_resp);
        if(res == WILDDOG_CONN_COAP_RESPON_IGN)
            return WILDDOG_ERR_NOERR;
        else if(res == WILDDOG_ERR_OBSERVEERR)
            return WILDDOG_ERR_OBSERVEERR;
            
        /*@ Separation of reply  do not remove */
        if( _wilddog_conn_coap_recvSeparationRespCheck(curr,p_resp) \
            == WILDDOG_CONN_COAP_RESPON_IGN )
            return WILDDOG_ERR_NOERR;
        /*
        **@ todo Fragmentation
        **@ get payload
