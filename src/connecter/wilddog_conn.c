...
#define FIRSTRTRANSMIT_INV  (2000)
...
#define WILDDOG_OBSERVE_REQ (0X01)
...
STATIC int _wilddog_conn_hostGet
    (
...
/*@ check auth status*/
inline void _wilddog_conn_authSet
    (
    Wilddog_Conn_T *p_conn,
...
STATIC Wilddog_Conn_AuthState_T _wilddog_conn_authGet
    (
    Wilddog_Conn_T *p_conn
    )
{
...
STATIC int _wilddog_conn_authSend(Wilddog_Repo_T *p_repo)
{
...
STATIC int _wilddog_conn_authDetec(Wilddog_Repo_T *p_repo)
{
    if(p_repo->p_rp_conn->d_auth_st == WILDDOG_CONN_AUTH_NOAUTH )
        {
            
            return _wilddog_conn_authSend(p_repo);
        }
    return 0;
}
    ...
    )
{
    ...
    {
        _wilddog_conn_authSet(p_conn,WILDDOG_CONN_AUTH_QURES,NULL);
...
inline void _wilddog_conn_node_addlist
    (
    Wilddog_Conn_T *p_conn,
...
STATIC void _wilddog_conn_payloadFree(Wilddog_Payload_T **pp_data)
{
...
STATIC int _wilddog_conn_cmd_onEvent
    (
    ...
    Wilddog_Conn_Node_T *p_conn_node)
{
    ...
    switch(p_conn_node->d_cmd)
    {
        case WILDDOG_CONN_CMD_ON:
            res = _wilddog_conn_cmd_onEvent(p_conn_node,p_arg->p_url->p_url_path);
            break;
...
{   
    if( cmd != WILDDOG_CONN_CMD_AUTH  &&
        _wilddog_conn_authGet(p_conn) != WILDDOG_CONN_AUTH_AUTHED)
            return 0;
    else 
    ...
    )
{
    ...
    */
    ...
    if( p_conn->d_remoteAddr.len == 0 )
        res = _wilddog_conn_hostGet( &p_conn->d_remoteAddr,
    ...
    res = _wilddog_conn_cmd(p_arg,p_conn,p_conn_node);

_CONN_SEND_FREE:

    _wilddog_conn_payloadFree(&p_payload);
    ...
    )
{
    ...
    if(RESP_IS_ERR(p_cn_recvData->d_RecvErr))
    {
        if(p_cn_recvData->d_RecvErr == WILDDOG_HTTP_UNAUTHORIZED)
            _wilddog_conn_authSet(p_conn,WILDDOG_CONN_AUTH_NOAUTH,NULL);
    ...
    switch(p_cn_node->d_cmd)
    {
        case WILDDOG_CONN_CMD_AUTH:
            if(d_cn_recvData.d_RecvErr == WILDDOG_HTTP_OK )
                _wilddog_conn_authSet(p_conn,WILDDOG_CONN_AUTH_AUTHED, \
...
/*
* define the ping interval with the server, in ms
*/
...
STATIC int _wilddog_conn_trySync(Wilddog_Repo_T *p_repo)
{
    ...
    res = _wilddog_conn_authDetec(p_repo);

    return res ;
    
}
/**/
/*@ call while creat repo 
**@ malloc conn_ct 
*/
Wilddog_Conn_T * _wilddog_conn_init(Wilddog_Repo_T* p_repo)
{
    ...
    res = _wilddog_conn_hostGet( &p_repo->p_rp_conn->d_remoteAddr,
                             p_repo->p_rp_url->p_url_host,WILDDOG_PORT);
    if(res >=0)
        _wilddog_conn_pkt_init(p_repo_conn->d_socketid,&p_repo_conn->d_remoteAddr);
    _wilddog_conn_authSend(p_repo);
    return p_repo_conn;
}
/*@ free the list and free p_repo->conn
**@
*/
