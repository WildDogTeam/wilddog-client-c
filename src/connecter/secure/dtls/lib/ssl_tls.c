...
#include "polarssl/polarssl_platform.h"
#else
...
int ssl_init( ssl_context *ssl )
{
    ...
    if( ssl->in_buf == NULL || ssl->out_buf == NULL )
    {
        ...
        ssl->in_buf = NULL;
        ssl->out_buf = NULL;
		wilddog_debug("len = %x, ssl->in_buf = %p, ssl->out_buf = %p",len, ssl->in_buf, ssl->out_buf);
        return( POLARSSL_ERR_SSL_MALLOC_FAILED );
    }
...
#endif
{
    ...
    if( ssl->state != SSL_HANDSHAKE_OVER )
    {
		wilddog_debug_level(WD_DEBUG_WARN, "rehandshark!");
        if( ( ret = ssl_handshake( ssl ) ) != 0 )
        {
            SSL_DEBUG_RET( 1, "ssl_handshake", ret );
            return( ret );
