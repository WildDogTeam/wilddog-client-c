	    ...
	    while(node)
	    {
	           ...
	           )
			{
				if(node->flag == OFF_FLAG)
				{
			        ...
			        tmp_arg->p_url->p_url_host= (Wilddog_Str_T *)wmalloc( \
						    strlen((const char *)node->p_url->p_url_host)+1);
			        ...
			        tmp_arg->p_url->p_url_path= (Wilddog_Str_T *)wmalloc( \
						strlen((const char *)node->p_url->p_url_path)+1);
