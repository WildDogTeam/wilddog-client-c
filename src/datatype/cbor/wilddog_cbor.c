...
STATIC s32 _wilddog_c2n_getItemLen(Wilddog_Payload_T* data, s32 *p_num)
{
    ...
    {
        wilddog_debug("cannot read header!\n");
        return 0;
    }
    return 0;
}
    ...
    )
{
    ...
    wilddog_assert((p_data->d_dt_len >= p_data->d_dt_pos));
    ...
    )
{
    ...
    *(p_data->p_dt_data + p_data->d_dt_pos) = \
    ...
    (p_data->d_dt_pos)++;
    if(_wilddog_n2c_uintAdditionalInfo(p_node->d_wn_len) == 24)
    {
        *(p_data->p_dt_data + p_data->d_dt_pos) = p_node->d_wn_len;
        (p_data->d_dt_pos)++;
    }
    else if(_wilddog_n2c_uintAdditionalInfo(p_node->d_wn_len) == 25)
    {
    ...
    )
{
    ...
    *(p_data->p_dt_data + p_data->d_dt_pos) = \
    ...
    (p_data->d_dt_pos)++;
    if(_wilddog_n2c_uintAdditionalInfo(len) == 24)
    {
        *(p_data->p_dt_data + p_data->d_dt_pos) = len;
        (p_data->d_dt_pos)++;
    }
    else if(_wilddog_n2c_uintAdditionalInfo(len) == 25)
    {
