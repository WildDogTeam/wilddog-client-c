...
STATIC Wilddog_Return_T _wilddog_ct_destroyRef(void *args, int flag)
{
    ...
    {
        wilddog_debug("cannot find repo!");
        if(NULL != (*p_ref)->p_ref_url)
        {
    ...
    )
{
    ...
    wilddog_assert(p_repo);
    wilddog_assert(p_url);
    if(NULL == p_repo->p_rp_head)
        return NULL;
...
STATIC Wilddog_Ref_T * _wilddog_ct_getRef(void* arg, int flag)
{
    ...
    if(WILDDOG_ERR_NOERR != _wilddog_url_getPath(p_srcRef->p_ref_url->p_url_path, \
        type, str, &p_path))
    {
        wilddog_debug("can not find path!");
        return NULL;
    }

    /*
     *  in child type, we should check whether the new path is valid, but it's hard,
     *  because user may call this func before get data from server, so let user 
     *  check it.
    */
    ...
    p_url->p_url_host = p_srcRef->p_ref_url->p_url_host;// for temp use.
...
STATIC Wilddog_Repo_T * _wilddog_ct_createRepo(Wilddog_Url_T *p_url)
{
    ...
    if(NULL == p_repo)
    {
        wilddog_debug("cannot malloc p_repo!\n");
        return NULL;
    }
